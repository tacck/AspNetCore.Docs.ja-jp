---
title: 'ASP.NET Core :::no-loc(Blazor Server)::: アプリをセキュリティで保護する'
author: guardrex
description: ':::no-loc(Blazor Server)::: アプリを ASP.NET Core アプリケーションとしてセキュリティで保護する方法について説明します。'
...
---
# <a name="secure-aspnet-core-no-locblazor-server-apps"></a><span data-ttu-id="c887f-103">ASP.NET Core :::no-loc(Blazor Server)::: アプリをセキュリティで保護する</span><span class="sxs-lookup"><span data-stu-id="c887f-103">Secure ASP.NET Core :::no-loc(Blazor Server)::: apps</span></span>
...
<span data-ttu-id="c887f-105">:::no-loc(Blazor Server)::: アプリは、ASP.NET Core アプリと同じ方法でセキュリティが構成されます。</span><span class="sxs-lookup"><span data-stu-id="c887f-105">:::no-loc(Blazor Server)::: apps are configured for security in the same manner as ASP.NET Core apps.</span></span> <span data-ttu-id="c887f-106">詳細については、<xref:security/index> の記事を参照してください。</span><span class="sxs-lookup"><span data-stu-id="c887f-106">For more information, see the articles under <xref:security/index>.</span></span> <span data-ttu-id="c887f-107">この概要に含まれるトピックは、:::no-loc(Blazor Server)::: に特に当てはまります。</span><span class="sxs-lookup"><span data-stu-id="c887f-107">Topics under this overview apply specifically to :::no-loc(Blazor Server):::.</span></span>

## <a name="no-locblazor-server-project-template"></a><span data-ttu-id="c887f-108">:::no-loc(Blazor Server)::: プロジェクト テンプレート</span><span class="sxs-lookup"><span data-stu-id="c887f-108">:::no-loc(Blazor Server)::: project template</span></span>

<span data-ttu-id="c887f-109">:::no-loc(Blazor Server)::: プロジェクト テンプレートは、プロジェクトの作成時に認証を構成できます。</span><span class="sxs-lookup"><span data-stu-id="c887f-109">The :::no-loc(Blazor Server)::: project template can be configured for authentication when the project is created.</span></span>
...
<span data-ttu-id="c887f-111">認証メカニズムを使用して新しい :::no-loc(Blazor Server)::: プロジェクトを作成するには、<xref:blazor/tooling> の Visual Studio のガイダンスに従ってください。</span><span class="sxs-lookup"><span data-stu-id="c887f-111">Follow the Visual Studio guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism.</span></span>

<span data-ttu-id="c887f-112">**[新しい ASP.NET Core Web アプリケーションを作成する]** ダイアログで **[:::no-loc(Blazor Server)::: アプリ]** テンプレートを選択した後、 **[認証]** の下の **[変更]** を選択します。</span><span class="sxs-lookup"><span data-stu-id="c887f-112">After choosing the **:::no-loc(Blazor Server)::: App** template in the **Create a new ASP.NET Core Web Application** dialog, select **Change** under **Authentication** .</span></span>
...
* <span data-ttu-id="c887f-115">**個人のユーザー アカウント** : ユーザーアカウントは次のように格納できます。</span><span class="sxs-lookup"><span data-stu-id="c887f-115">**Individual User Accounts** : User accounts can be stored:</span></span>
  * <span data-ttu-id="c887f-116">ASP.NET Core の [:::no-loc(Identity):::](xref:security/authentication/identity) システムを使用するアプリ内。</span><span class="sxs-lookup"><span data-stu-id="c887f-116">Within the app using ASP.NET Core's [:::no-loc(Identity):::](xref:security/authentication/identity) system.</span></span>
...
<span data-ttu-id="c887f-121">認証メカニズムを使用して新しい :::no-loc(Blazor Server)::: プロジェクトを作成するには、<xref:blazor/tooling> の Visual Studio Code のガイダンスに従ってください。</span><span class="sxs-lookup"><span data-stu-id="c887f-121">Follow the Visual Studio Code guidance in <xref:blazor/tooling> to create a new :::no-loc(Blazor Server)::: project with an authentication mechanism:</span></span>
...
| `Individual`             | <span data-ttu-id="c887f-127">:::no-loc(ASP.NET Core Identity)::: を使用してアプリに格納されているユーザー</span><span class="sxs-lookup"><span data-stu-id="c887f-127">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
1. <span data-ttu-id="c887f-138">**[新しい :::no-loc(Blazor Server)::: アプリの構成]** ステップで、 **[認証]** ドロップダウンから **[Individual Authentication (in-app)]\(個別認証 (アプリ内)\)** を選択します。</span><span class="sxs-lookup"><span data-stu-id="c887f-138">On the **Configure your new :::no-loc(Blazor Server)::: App** step, select **Individual Authentication (in-app)** from the **Authentication** drop down.</span></span>

1. <span data-ttu-id="c887f-139">:::no-loc(ASP.NET Core Identity)::: を使用してアプリに格納されている個々のユーザーに対して、アプリが作成されます。</span><span class="sxs-lookup"><span data-stu-id="c887f-139">The app is created for individual users stored in the app with :::no-loc(ASP.NET Core Identity):::.</span></span>
...
<span data-ttu-id="c887f-141">コマンド シェルで次のコマンドを使用し、認証メカニズムを使って新しい :::no-loc(Blazor Server)::: プロジェクトを作成します。</span><span class="sxs-lookup"><span data-stu-id="c887f-141">Create a new :::no-loc(Blazor Server)::: project with an authentication mechanism using the following command in a command shell:</span></span>
...
| `Individual`             | <span data-ttu-id="c887f-147">:::no-loc(ASP.NET Core Identity)::: を使用してアプリに格納されているユーザー</span><span class="sxs-lookup"><span data-stu-id="c887f-147">Users stored in the app with :::no-loc(ASP.NET Core Identity):::</span></span> |
...
* <span data-ttu-id="c887f-157">コマンド シェルで :::no-loc(Blazor Server)::: テンプレート (`blazorserver`) のヘルプ コマンドを実行します。</span><span class="sxs-lookup"><span data-stu-id="c887f-157">Execute the help command for the :::no-loc(Blazor Server)::: template (`blazorserver`) in a command shell:</span></span>
...
## <a name="scaffold-no-locidentity"></a><span data-ttu-id="c887f-158">スキャフォールディング :::no-loc(Identity):::</span><span class="sxs-lookup"><span data-stu-id="c887f-158">Scaffold :::no-loc(Identity):::</span></span>

<span data-ttu-id="c887f-159">:::no-loc(Identity)::: を :::no-loc(Blazor Server)::: プロジェクトにスキャフォールディングします。</span><span class="sxs-lookup"><span data-stu-id="c887f-159">Scaffold :::no-loc(Identity)::: into a :::no-loc(Blazor Server)::: project:</span></span>
