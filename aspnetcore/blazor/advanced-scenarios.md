...
* `maxBase64SegmentSize` は `8192` に設定されます。これは `maxBase64SegmentSize = segmentSize * 4 / 3` から計算されます。
* 低レベルの .NET Core メモリ管理 API は、`uploadedSegments` のサーバーにメモリ セグメントを格納するために使用されます。
