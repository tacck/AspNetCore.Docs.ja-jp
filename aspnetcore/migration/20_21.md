...
ASP.NET Core 2.1 では、 [ Razor クラスライブラリ](xref:razor-pages/ui-class)(rcl) として[ASP.NET Core Identity ](xref:security/authentication/identity)が提供されています。
...
Identity2.1 は、領域と共にエンドポイントを公開 `Identity` します。 たとえば、次の表は、 Identity 2.0 から2.1 に変更されるエンドポイントの例を示しています。
...
### <a name="update-identity-to-version-21"></a>Identityバージョン2.1 に更新します。
...
### <a name="replace-identity-20-ui-with-the-identity-21-razor-class-library"></a>Identity2.0 UI を Identity 2.1 Razor クラスライブラリに置き換える
...
## <a name="changes-to-razor-pages-projects-razor-files"></a>Razorページプロジェクトファイルの Razor 変更
...
## <a name="changes-to-mvc-projects-razor-files"></a>MVC プロジェクトファイルの変更点 Razor
