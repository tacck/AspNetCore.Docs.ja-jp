...
1. `wwwroot/index.html`の場合:

   * スコープ付き `<link>` スタイルの要素に要素を追加 `<head>` します。
   * コンポーネントを読み込む要素を変更 `App` します。
...
1. では、 `Shared/MainLayout.razor` 要素内のすべてのコンテンツをで囲み `<div>` `id` `page` ます。
...
1. 最新の基本 `wwwroot/css/app.css` ファイルには、次のスタイルが含まれています。 次のスタイルとアプリに追加したものを残して、余分なスタイルを削除します。

    `wwwroot/css/app.css` (基本スタイルには、開発者が追加したスタイルは含まれません)。
