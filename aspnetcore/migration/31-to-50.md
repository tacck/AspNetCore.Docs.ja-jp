...
ファイル [ のglobal.js](/dotnet/core/tools/global-json) に依存して特定の .NET Core SDK バージョンを対象とする場合は、 `version` インストールされている .net 5.0 SDK バージョンにプロパティを更新します。 たとえば、次のように入力します。
...
>
> オプションをに設定するか、設定を解除したままにすることができ `false` ます。このオプションは、既定では `false` 、 *旧バージョンとの互換性のためだけに用意* されています。
>
