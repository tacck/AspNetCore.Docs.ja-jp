...
* SameSite ヘッダーのない cookie は、 `SameSite=Lax`既定でとして扱われます。
* `SameSite=None`クロスサイトクッキーの使用を許可するには、を使用する必要があります。
* アサート`SameSite=None`する cookie は、として`Secure`もマークされている必要があります。
* [`<iframe>`](https://developer.mozilla.org/docs/Web/HTML/Element/iframe)を使用するアプリケーションでは、 `sameSite=Lax`クロス`sameSite=Strict`サイトシナリオ`<iframe>`として扱われるため、またはクッキーで問題が発生する可能性があります。
* この値`SameSite=None`は[2016 標準](https://tools.ietf.org/html/draft-west-first-party-cookies-07)では許可されていないため、一部の実装`SameSite=Strict`ではとしてこのような cookie を扱います。 このドキュメントの「[古いブラウザーのサポート](#sob)」を参照してください。
...
.NET Core 3.0 では、更新された SameSite 値がサポートさ`SameSiteMode.Unspecified`れ、 `SameSiteMode`列挙型に追加の列挙値が追加されます。
...
.NET Framework と .NET Core 2.1 の特定の動作変更は、 `SameSite`プロパティが値を`None`解釈する方法です。 パッチの前に、" `None`属性をまったく出力しない" という値が適用される前に、パッチの後に "値`None`を持つ属性を生成する" という意味になります。 Patch の後に`SameSite`の`(SameSiteMode)(-1)`値を指定すると、属性は出力されません。

フォーム認証およびセッション状態クッキーの既定の SameSite 値は、から`None`に`Lax`変更されました。

::: moniker-end

## <a name="api-usage-with-samesite"></a>SameSite を使用した API の使用

[HttpContext](xref:Microsoft.AspNetCore.Http.IResponseCookies.Append*) 。既定値はに`Unspecified`なります。つまり、クッキーに SameSite 属性が追加されず、クライアントは既定の動作を使用します (新しいブラウザーではなく、古いブラウザーではありません)。 次のコードは、cookie の SameSite 値をに変更`SameSiteMode.Lax`する方法を示しています。
...
* の`SameSiteMode.None`動作を再定義してを生成します。`SameSite=None`
* 新しい値`SameSiteMode.Unspecified`を追加して、SameSite 属性を省略します。
* すべての cookie Api は`Unspecified`、既定でに設定されます。 Cookie を使用する一部のコンポーネントでは、そのシナリオにより具体的な値が設定されています。 例については、上の表を参照してください。

::: moniker-end

::: moniker range=">= aspnetcore-3.0"

ASP.NET Core 3.0 以降では、一貫性のないクライアントの既定値と競合しないように、SameSite の既定値が変更されました。 次の Api は、既定のを`SameSiteMode.Lax `から`-1`に変更し、これらの cookie の SameSite 属性を出力しないようにしました。
...
SameSite サポートは、2.0 の ASP.NET Core で最初に[2016 ドラフト標準](https://tools.ietf.org/html/draft-west-first-party-cookies-07#section-4.1)を使用して実装されました。 2016標準はオプトインでした。 ASP.NET Core、既定で複数の cookie をに`Lax`設定することによってオプトインします。 認証でいくつかの[問題](https://github.com/aspnet/Announcements/issues/318)が発生すると、ほとんどの SameSite の使用が[無効に](https://github.com/aspnet/Announcements/issues/348)なりました。
...
* Cookie を既定でと`SameSite=Lax`して扱うことを指定します。
* クロスサイト配信を有効`SameSite=None`にするために明示的にアサートするクッキーをと`Secure`してマークする必要があることを指定します。 `None`は、オプトアウトする新しいエントリです。
...
2016 SameSite 標準では、不明な値を値と`SameSite=Strict`して扱う必要があります。 2016 SameSite 標準をサポートする古いブラウザーからアクセスされるアプリは、値がの`None`SameSite プロパティを取得すると破損する可能性があります。 Web apps が古いブラウザーをサポートする予定の場合は、ブラウザーの検出を実装する必要があります。 ユーザーエージェントの値が変動し、頻繁に変更されるため、ASP.NET Core はブラウザーの検出を実装しません。 の<xref:Microsoft.AspNetCore.CookiePolicy>拡張ポイントでは、ユーザーエージェント固有のロジックをプラグインできます。

で`Startup.Configure`は、または cookie <xref:Microsoft.AspNetCore.Builder.CookiePolicyAppBuilderExtensions.UseCookiePolicy*>を書き込む<xref:Microsoft.AspNetCore.Builder.AuthAppBuilderExtensions.UseAuthentication*> *任意の*メソッドを呼び出す前にを呼び出すコードを追加します。
...
前の例では`MyUserAgentDetectionLib.DisallowsSameSiteNone` 、は、ユーザーエージェントが SameSite `None`をサポートしていないかどうかを検出する、ユーザーが指定したライブラリです。

[!code-csharp[](samesite/sample/Startup31.cs?name=snippet2)]

次のコードは、サンプル`DisallowsSameSiteNone`メソッドを示しています。
...
Chrome 78 + には一時的な軽減策があるため、誤解を招く結果になります。 Chrome 78 + 一時的な軽減策では、2分前よりも少ない cookie を使用できます。 適切なテストフラグが有効になっている Chrome 76 または77では、より正確な結果が得られます。 新しい SameSite の動作をテストする`chrome://flags/#same-site-by-default-cookies`には、[**有効**に切り替えます。 新しい`None`設定で失敗するように、Chrome の旧バージョン (75 以降) が報告されます。 このドキュメントの「[古いブラウザーのサポート](#sob)」を参照してください。
...
カナリアバージョン`80.0.3975.0`以降では、テストのために、厳密でない + POST の一時的な軽減策`--enable-features=SameSiteDefaultChecksMethodRigorously`を無効にすることができます。この場合、新しいフラグを使用して、軽減策が削除された機能の最終的な終了状態でサイトとサービスをテストできます。 詳細については、「Chromium Projects [SameSite Updates](https://www.chromium.org/updates/same-site) 」を参照してください。

### <a name="test-with-safari"></a>Safari を使用したテスト

Safari 12 では、以前のドラフトが厳密に実装`None`されており、新しい値が cookie に含まれていると失敗します。 `None`このドキュメントでは、以前の[ブラウザーをサポート](#sob)するブラウザーの検出コードを使用して回避します。 MSAL、ADAL、使用している任意のライブラリを使用して、Safari 12、Safari 13、WebKit ベースの OS スタイルのログインをテストします。 この問題は、基盤の OS バージョンによって変わります。 OSX Mojave (10.14) と iOS 12 には、新しい SameSite の動作に互換性の問題があることがわかっています。 OS を OSX Catalina.properties (10.15) または iOS 13 にアップグレードすると、問題が解決されます。 Safari には、現在、新しい仕様動作をテストするオプトインフラグがありません。

### <a name="test-with-firefox"></a>Firefox でのテスト

新しい標準の Firefox サポートは、バージョン68以降で、機能フラグ`about:config` `network.cookie.sameSite.laxByDefault`を使用してページをオンにすることでテストできます。 以前のバージョンの Firefox との互換性に関する問題は報告されていません。
...
`edge://flags/#same-site-by-default-cookies`ページに SameSite フラグが設定されています。 Edge Chromium で互換性の問題は検出されませんでした。
...
| サンプル               | ドキュメント |
| ----------------- | ------------ |
| [.NET Core Razorページ](https://github.com/blowdart/AspNetSameSiteSamples/tree/master/AspNetCore31RazorPages)  | <xref:security/samesite/rp31> |
