...
<span data-ttu-id="7e9d7-189">前のコードでは、を呼び出すことで、 `AddNewtonsoftJson` 次の WEB API、MVC、および Pages の各機能を使用するように構成されてい Razor `Newtonsoft.Json` ます。</span><span class="sxs-lookup"><span data-stu-id="7e9d7-189">In the preceding code, the call to `AddNewtonsoftJson` configures the following Web API, MVC, and Razor Pages features to use `Newtonsoft.Json`:</span></span>
